				...
				this.fallInterval = setInterval(() => {
					...
					for (let neighbour of neighbours) {
						if (neighbour.y > this.y) {
							if (this.fallcount > 0) {
								if (isPlayer(neighbour) || isEnemy(neighbour))
									neighbour.hit();
									...
									if (neighbour.type === "mwall") {
										...
										if (
											...
											if (
												this.board[neighbour.y + 1][
													neighbour.x
												].type === "clear"
											) {
												this.mwallFunc.bind(this)(
													neighbour
												);
											}
											this.delete();
						...
						if (
							(neighbour.y > this.y || // is below
								(neighbour.y === this.y && // is the same y
									this.board[neighbour.y + 1][neighbour.x]
										.type === "clear" &&
									isSlippery(
										this.board[this.y + 1][this.x]
									))) && // below is slippery :DD
